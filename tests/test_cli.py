...
def invoke(*args):
    ...
    return subprocess.run([sys.executable, '-m', 'understudy', *map(str, args)],
...
def test_run_writes_real_clinic_evidence_and_compare_catches_seeded_fault(tmp_path, monkeypatch):
    import json
    ...
    class ScriptedCompletion:
        ...
        def __call__(self, messages, *, json_output=False):
            ...
            if self.role == 'customer':
                return '<END>'
            if self.role == 'judge':
                return json.dumps({'scores': {d: 4 for d in RUBRIC},
                                   'explanations': {d: 'Scripted test score, not model evidence.' for d in RUBRIC}})
            return json.dumps({'reply': 'I can arrange that.', 'action': {
                'name': 'book', 'arguments': {'name': 'Ava Stone',
    ...
    assert any(check.id == 'confirmation-before-booking' and check.status == 'fail'
    ...
    def malformed_target(self, messages, **kwargs):
        return 'not JSON' if self.role == 'target' else original_call(self, messages, **kwargs)
