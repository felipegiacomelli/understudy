...
def test_completion_records_actual_usage_and_provider_specific_request():
    ...
    complete = Completer(
        "customer", "deepseek-flash", "https://api.deepseek.com", client=client
    )
...
def test_openai_request_omits_deepseek_parameters_and_unknown_usage_is_null():
    ...
    complete = Completer(
        "target", "gpt-4.1-mini-2025-04-14", "https://api.openai.com/v1", client=client
    )
...
def test_client_is_created_only_on_call_with_no_hidden_retries(monkeypatch):
    ...
    client = Client(response())
    monkeypatch.setattr(
        openai, "OpenAI", lambda **kwargs: constructed.append(kwargs) or client
    )
