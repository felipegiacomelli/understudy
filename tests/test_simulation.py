...
from understudy.records import Observation, Scenario
...
def booking_decisions():
    return Decisions(
        {"reply": "Please confirm the details", "action": {"name": "request_confirmation", "arguments": BOOKING}},
        {"reply": "Booked", "action": {"name": "book", "arguments": BOOKING}},
    )


def test_confirmation_is_bound_to_prior_summary_and_fault_is_detected():
    declined = scenario(id="declined", expected_outcome="active", expected_terminal_states=[])
    decisions = Decisions(
        {"reply": "Confirm?", "action": {"name": "request_confirmation", "arguments": BOOKING}},
    ...
    faulty = run_scenario(
        declined, ClinicTarget(decisions, fault="premature_booking"), Customer("NO", "<END>")
    )
...
def test_confirmation_does_not_authorize_substituted_slot():
    ...
    record = run_scenario(scenario(expected_outcome="active", expected_terminal_states=[]), ClinicTarget(decisions), Customer("CONFIRM", "<END>"))
...
def test_booking_rejects_missing_wrong_typed_and_unavailable_fields(arguments, reason):
    decisions = Decisions({"reply": "Trying", "action": {"name": "book", "arguments": arguments}})
    record = run_scenario(scenario(expected_outcome="active", expected_terminal_states=[]), ClinicTarget(decisions, fault="premature_booking"), Customer("<END>"))
...
def test_cancel_then_rebook_is_not_stopped_at_cancellation():
    decisions = Decisions(
        {"reply": "Cancelled", "action": {"name": "cancel", "arguments": {"appointment_id": "APT-1"}}},
        {"reply": "Confirm new booking", "action": {"name": "request_confirmation", "arguments": BOOKING}},
        {"reply": "Rebooked", "action": {"name": "book", "arguments": BOOKING}},
    )
    target = ClinicTarget(decisions, appointments=[{**BOOKING, "appointment_id": "APT-1"}])
    record = run_scenario(scenario(id="rebook", expected_outcome="booked", max_exchanges=5), target, Customer("new slot", "CONFIRM"))
    assert [turn.observation.state["status"] for turn in record.turns] == ["cancelled", "active", "booked"]


def test_handoff_probe_expects_silence_and_empty_ordinary_reply_fails_check():
    handoff = scenario(id="handoff", expected_outcome="handed_off", expected_terminal_states=["handed_off"], post_terminal_probe="Are you there?")
    record = run_scenario(handoff, ClinicTarget(Decisions({"reply": "A human will help", "action": {"name": "handoff", "arguments": {}}})), Customer())
    assert record.turns[-1].customer_message == "Are you there?"
    assert record.turns[-1].observation.reply == ""
    assert all(result.status == "pass" for result in evaluate_checks(record))

    empty = run_scenario(scenario(expected_outcome="active", expected_terminal_states=[]), ClinicTarget(Decisions({"reply": ""})), Customer("<END>"))
    assert any(result.id == "expected-replies" and result.status == "fail" for result in evaluate_checks(empty))


def test_runner_handles_exact_sentinel_limit_errors_and_fresh_suite_instances():
    stopped = run_scenario(scenario(expected_outcome="active", expected_terminal_states=[]), ClinicTarget(Decisions({"reply": "ok"})), Customer("<END>"))
    assert stopped.closure_reason == "customer-ended"

    limited = run_scenario(scenario(max_exchanges=1, expected_outcome="active", expected_terminal_states=[]), ClinicTarget(Decisions({"reply": "ok"})), Customer("not <END>"))
    assert limited.closure_reason == "exchange-limit"

    errored = run_scenario(scenario(), ClinicTarget(Decisions(RuntimeError("secret details"))), Customer())
    ...
    def target_factory(_scenario):
        ...
        return target
    run = run_suite([scenario(id="one", expected_outcome="active", expected_terminal_states=[]), scenario(id="two", expected_outcome="active", expected_terminal_states=[])], target_factory, lambda _: Customer("<END>"))
...
def test_missing_or_inferred_state_is_unsupported_and_claims_do_not_prove_success():
    record = run_scenario(scenario(), ClinicTarget(Decisions({"reply": "Booked!"})), Customer("<END>"))
    record.turns[0].observation.state_source = "inferred"
    assert any(result.status == "unsupported" for result in evaluate_checks(record))
    assert any(result.id == "final-outcome" and result.status != "pass" for result in evaluate_checks(record))
