...
from .records import JsonValue, Scenario, Turn, load_run, save_run
...
class LiveDecision:
    ...
    def __call__(
        self, history: list[dict[str, str]], state: dict[str, JsonValue]
    ) -> dict[str, JsonValue]:
        ...
        raw = self.complete(
            [{"role": "system", "content": instructions}, *history], json_output=True
        )
        try:
            decision = json.loads(
...
class LiveCustomer:
    def __init__(self, scenario: Scenario, complete: Completer):
...
def run_live(output: Path, fault: str | None) -> int:
    ...
    def target_factory(scenario):
        appointments = (
            ...
            if scenario.id == "cancel-rebook"
            else []
        )
        return ClinicTarget(
            LiveDecision(target), fault=fault, appointments=appointments
        )
    ...
    try:
        run = run_suite(
            SCENARIOS,
            target_factory,
            lambda s: LiveCustomer(s, customer),
            role_settings={
                c.role: {
                    "model": c.model,
                    "base_url": c.base_url,
            ...
            rubric=EVALUATION_POLICY,
            target_revision="clinic-v1",
            fault=fault,
        )
        ...
        run.usage = sorted(
            (call for c in clients for call in c.calls),
        ...
        return (
            ...
            else (0 if all(s == "pass" for s in statuses) else 1)
        )
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    compare = commands.add_parser(
        "compare", help="Compare stored results; no API calls"
    )
    ...
    try:
        ...
        if any(
            scenario_status(record) == "error"
            for record in comparison.candidate.records
        ):
            return 2
